...
async function getAllTablesStructure(databaseName) {
  ...
  const groupedTableInfo = tableInfo.reduce((acc, row) => {
    ...
    const defaultValue =
      (row.column_default?.toString()?.toLowerCase() || "null") !== "null"
        ? row.column_default.replace(/(?<=\w)'|'(?=\w)/g, "")
        : "";

    const autoIncrement = row.column_key === "PRI" ? "PRI" : null;
...
router.get("/getModelDetail", async (req, res) => {
  ...
  try {
    ...
    if (model) {
      ...
      return res.status(200).send({ status: false, msg: "Model not found" });
    }
  } catch (err) {
    console.error("Internal Server Error", err);
    return res
